...
const outputPath =
  process.env.ZKEHR_SCENARIO4_BATCHED_LOG_PATH ||
  "evaluation/concurrent_batched_results.csv";
...
async function runBatches(batchSize = 10, total = 50, delayMs = 1000) {
  const totalBatches = Math.ceil(total / batchSize);
  let current = 1;

  for (let b = 0; b < totalBatches; b++) {
    console.log(`\n🚀 Starting batch ${b + 1}/${totalBatches}`);
    const tasks = [];
    for (let i = 0; i < batchSize && current <= total; i++, current++) {
      tasks.push(verifyInstance(current));
    }
  ...
  console.log("\n✅ All batched requests complete.");
}

runBatches(
  Number(process.env.ZKEHR_SCENARIO4_BATCH_SIZE || 10),
  Number(process.env.ZKEHR_SCENARIO4_TOTAL_REQUESTS || 50),
  Number(process.env.ZKEHR_SCENARIO4_DELAY_MS || 1000)
);
