#!/usr/bin/env python3
"""TEMPLATE ONLY: IBM Quantum toy policy hardware execution for a later phase.
...
def build_toy_policy_circuit_for_hardware():
    import sys

    toy_dir = REPO_ROOT / "quantum-experiments" / "qiskit" / "toy_access_policy"
...
def submit_toy_policy_hardware_job_TEMPLATE(explicit_user_approval: bool = False):
    """Template placeholder for Phase 3.5.
    ...
    # Future Phase 3.5 adaptation point:
    # from qiskit_ibm_runtime import QiskitRuntimeService, SamplerV2 as Sampler
    # token = get_token_from_environment()
    # service = QiskitRuntimeService(channel="ibm_quantum", token=token)
    # backend = service.least_busy(operational=True, simulator=False, min_num_qubits=5)
    # circuit = build_toy_policy_circuit_for_hardware()
    # sampler = Sampler(mode=backend)
    # job = sampler.run([circuit], shots=1024)
    # return job.job_id()

    raise NotImplementedError("Adapt this template in Phase 3.5 before hardware execution.")


if __name__ == "__main__":
    print("TEMPLATE ONLY: no IBM job was submitted.")
    print("Use this file only after Phase 3.3 reports IBM_SERVICE_READY and the user approves Phase 3.5 hardware execution.")
